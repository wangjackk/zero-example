...
> 框架级 API 参考 [../framework/02-routine.md](../framework/02-routine.md),这里只讲项目特有约定。
...
`True` = kernel 连接后 auto-start,常驻单实例。**只用于基础设施 / manager**(参见 [01-structure.md](./01-structure.md#passive-routine-的角色))。

```python
class HttpServer(Routine):
    is_passive = True   # kernel auto-start,常驻 HTTP 前门
```
...
class WaitInput(BaseModel):
    """WAIT input: duration seconds then return."""
...
详见 [../framework/06-errors.md](../framework/06-errors.md)。
...
业务编排走 `zero.shell.Shell`,不用裸 `ctx.submit+start`:
...
async def run(self, kwargs):
    shell = Shell(self)
    await shell.push('ui_noop', {'n': 'first'})
    await shell.push('quick', {'msg': 'parallel'})
    shell.complete()
...
`zero` 的 UI 交互**走 wire**:routine 经 `ctx.req(http_server_id, 'ui_request', ...)`
把请求发给 `HttpServer` 的 `@request('ui_request')` handler(集成原 AgentWSBridge),
HttpServer 广播 `ui_request` 给前端,等前端 `ui_response` 后返回回执。不散落各 routine
...
- **跨 routine 共享状态走 wire**——UI 请求的 pending future 是 `HttpServer` 实例字段(`_pending_ui`),不是模块级全局变量;HttpServer 停止时随 instance GC 自动回收。
- **不吞超时**——审批 / 投票类必须 fail-closed。

## per-agent workspace 隔离

agent 子系统(claudecode / react_agent)的 skill / tool routine 通过 `WORKSPACE_KEY` 拿 agent workspace 路径,实现 per-agent 隔离:

```python
from ..tools._paths import WORKSPACE_KEY

async def run(self, kwargs: Dict[str, Any]):
    workspace = kwargs.get(WORKSPACE_KEY)
    # workspace = <project>/.agents/<agent_id>/
    ...
```

约定:
- **per-agent 数据**(`<project>/.agents/<agent_id>/`)放在 workspace 下,不污染全局。
- **per-agent routine** 用 `agent_id` 前缀 name(`agent_a/list_skills`)避免全局重名,实例化时绑定 workspace。
- **跨 agent 共享数据** 走全局路径(`runtime/claudecode/` 等)。
...
```python
routine = SearchSkill()
result = await routine.run({WORKSPACE_KEY: '/tmp/test', 'query': 'git'})
```

需要 ctx 的(编排 / 子 routine)走 `RoutineHub` + 真实 kernel,或 mock ctx。详见 [../framework/04-context.md](../framework/04-context.md)。
