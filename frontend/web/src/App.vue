<template>
  ...
  <div class="app">
    <header class="header">
      ...
      <NButton
        size="small"
        :type="showDag ? 'primary' : 'default'"
        :ghost="showDag"
        quaternary
        title="DAG 可视化"
        @click="showDag = !showDag"
      >⬡</NButton>
    </header>

    <Teleport to="body">
      <div v-show="showRunner" class="xr-overlay" @click.self="showRunner = false">
        <XmlRunner class="xr-float" :routines="routines" @run="onRun" />
      </div>
    </Teleport>

    <Teleport to="body">
      <div v-show="showDag" class="xr-overlay" @click.self="showDag = false">
        <DagPanel
          class="dag-float"
          :http-base="httpBase"
          :request="request"
      ...
      <main class="main">
        ...
        <div v-if="viewMode === 'create'" class="create-view">
          ...
          <div class="create-body">
            <NTabs v-model:value="createTab" type="segment" size="small">
              ...
              <NTabPane name="reactor" tab="Reactor">
                <ReactorCreateForm
                  :http-base="httpBase"
                  :project-suggestions="projectSuggestions"
                  :create-agent="createAgent"
...
import { NConfigProvider, NButton, NBadge, NTag, NTabs, NTabPane, NCard, NAlert, darkTheme } from 'naive-ui'
import type { GlobalThemeOverrides } from 'naive-ui'
import AgentPanel from './components/AgentPanel.vue'
import PromptPanel from './components/PromptPanel.vue'
import XmlRunner from './components/XmlRunner.vue'
...
import ClaudeCreateForm from './components/ClaudeCreateForm.vue'
import ReactCreateForm from './components/ReactCreateForm.vue'
import ReactorCreateForm from './components/ReactorCreateForm.vue'
import PrimeCreateForm from './components/PrimeCreateForm.vue'
import type { RoutineInfo } from './components/XmlRunner.vue'
...
// HTTP base for /routines + /builtin_skills + /agents + /dags 端点拉可选项.
// 默认 7780, 跟 web_server.py 默认 ZERO_HTTP_ADDR 一致.
...
// 'main' 已删除: sidebar 只列用户创建的 agent, 无内置 main 项.
...
// sys_prompt 现已按 agent 分: 每个 agent 自己的 epoch + messages.
// 顶层 promptEpoch/promptMessages 删除, 改用 agentEpochs/agentPrompts.
...
const createTab = ref<'claude' | 'react' | 'reactor' | 'prime'>('claude')

// 项目历史 (从 useAgents 拉, 给 ClaudeCreateForm 的 project_dir 自动补全)
...
function getAgentName(id: string): string {
  ...
  if (a) {
    const prefix = a.kind === 'react' ? 'React-' : a.kind === 'reactor' ? 'Reactor-' : a.kind === 'prime' ? 'Prime-' : 'Claude-'
    return prefix + a.agent_id.slice(0, 8)
  }
...
function mergeStreamText(current: string, incoming: string, isFinal: boolean) {
  ...
  // claudecode emits cumulative text; older agents may emit deltas. Support both.
...
async function onRun(
  payload: { format: 'form' | 'xml', name?: string, kwargs?: Record<string, unknown>, xml?: string },
  onResult: (data: unknown[]) => void,
  onError: (msg: string) => void,
  onDone: () => void,
) {
  try {
    // 两条路径都走 HTTP,统一以 user 身份调用:
    //   form → POST /agents/user/run/{routine_name} body=kwargs
    //   xml  → POST /agents/user/run/run_xml         body={xml}
    // 不设客户端超时:可能等用户交互(DAG approval / ui_request),固定超时会误杀.
    const url = payload.format === 'form'
      ? `${httpBase}/agents/user/run/${encodeURIComponent(payload.name ?? '')}`
      : `${httpBase}/agents/user/run/run_xml`
    const body = payload.format === 'form'
      ? (payload.kwargs ?? {})
      : { xml: payload.xml }
    ...
    if (payload.format === 'form') {
      const rname = payload.name ?? ''
      onResult([{ name: rname, result: data.result }])
    } else {
      // run_xml 返回的 result 可能是多 routine 结果数组
      onResult((data.result as unknown[] | undefined) ?? [])
    }
  } catch (e) {
