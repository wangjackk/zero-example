"""ListRunningAgents -- 列出所有 live agent (跨 claude/react/reactor/prime + user).

向各 manager (claude_code_agents / react_agents / reactor_agent_manager /
prime_agent_manager) 发 list_agents req, 合并 live 的, 返回 agent_id + rid +
agent_type. manager rid 通过 get_running_routines 按 name 找 (不依赖 bridge).
另外检查 user_agent passive routine 是否在运行, 在则加入列表.

用法:
    run_routine({name: 'list_running_agents'})
    # 返回: {'agents': [{'agent_id': 'claude_1', 'rid': '...', 'agent_type': 'claude'}, ...]}
"""
...
_MANAGERS = [
    ('claude_code_agents', 'claude'),
    ('react_agents', 'react'),
    ('reactor_agent_manager', 'reactor'),
...
class ListRunningAgents(Routine):
    """列出所有运行中的 agent (claude + react + reactor + prime)."""

    meta: ClassVar[Dict[str, Any]] = {
        'description': '列出所有运行中的 agent (claude + react + reactor + prime). 返回 agent_id + rid + agent_type.',
