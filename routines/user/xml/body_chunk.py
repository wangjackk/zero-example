"""XmlRoutine body hook 用的数据类型(对标老版 routine3.BodyChunk + zero TextChunk).

kshell 内部驱动跟老版不同(消息驱动 on_message+reorder,不是 wire body.chunk),
但 hook 签名/类型对齐老版--子类 override on_body_chunk/on_body_text 的写法一致.
"""
...
class BodyChunkKind(Enum):
    """body 流事件种类(三事件合一,对标老版 BodyChunkKind)."""
...
@dataclass
class BodyChunk:
    """统一的 body 流帧(对标老版 routine3.BodyChunk,精简掉 wire 字段 id/is_final).
...
@dataclass
class TextChunk:
    """顶层裸文本事件(对标老版 zero/routines/core/routine.py TextChunk).
