...
class ReactAgent(Routine):
    ...
    def __init__(self) -> None:
        ...
        # 在 _build_messages 里拼到 system prompt 后面 (对标 claudecode).
    ...
    async def on_started(self) -> None:
        ...
        # --- per-session workspace + skill seed (复用 claudecode 的 registry) ---
    ...
    async def _register_with_bridge(self) -> None:
        ...
        async def _on_success() -> None:
            ...
            # (same link claudecode uses; is_new = no prior messages).
    ...
    async def _build_messages(self) -> tuple[str | None, list[dict], str | None, list[dict]]:
        ...
        if routines:
            system = system + '\n\n## 可用 routine\n\n' + routines
        # 一级 skill 摘要注入 (对标 claudecode build_system_prompt 的 skill_summaries):
