...
Mirrors claudecode/manager.py. Agent records persist in the sqlite Memory
...
# OpenViking 默认配置 (跟 claudecode/manager.py 一致, 临时写死, 后续走配置文件).
# 前端 create_agent 未传 ov_config 时用此默认, 保证长期记忆默认启用.
...
class ReactAgents(Routine):
    ...
    async def _spawn_child(
        ...
        ov_config 默认 _DEFAULT_OV_CONFIG (保证长期记忆默认启用);
