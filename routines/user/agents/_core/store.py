"""Store -- sqlite backend for claudecode history + agent records.
...
continuous as it moves between projects. Modeled on
react_agent.memory.Memory: one background writer thread + queue (appends
never block the async loop), flush() barrier before reads, RLock + _tx()
...
class Store:
    """sqlite store for claudecode history + agent records.
    ...
    def __init__(
        ...
        *,
        ...
        self._writer = threading.Thread(
            target=self._write_loop, name='claudecode-store-writer', daemon=True,
        )
        self._writer.start()
        ...
        if last_compaction is not None:
            ...
            if summary:
                items.append({
                    ...
                    # 压缩生成的 summary 没有原始 uuid, 用 compaction row id 派生
                    '_ov_id': f'compact_{last_compaction_row_id}',
                })
            for row in rows:
                row_id = int(row['id'])
    ...
    def next_agent_id(self, prefix: str = 'claude') -> str:
        """生成下一个自增 agent_id: ``<prefix>_<N>``.

        N = 当前 DB 中以 ``<prefix>_`` 开头且后缀为纯数字的 agent_id 的最大数字 + 1.
        用 prefix 区分 kind (claude_1 / claude_2 ...), 便于 agent 间互相称呼.
        """
        self.flush()
        pfx = f'{prefix}_'
        with self._tx() as c:
...
def _append_item(items: list[dict[str, Any]], entry: dict[str, Any]) -> None:
    """把 entry 转成 LLM input item 并注入 _ov_id (OV 推送对齐用).

    只处理 user/assistant/function_call/function_call_output 四种.
    其他 (response_checkpoint/session_*/compaction) 跳过.
    _ov_id 取 entry 的 uuid (call_id 派生或随机), 压缩不变.
    """
    typ = entry.get('type')
    ov_id = str(entry.get('uuid') or '')
    if typ == 'user':
        items.append({
            'role': 'user',
