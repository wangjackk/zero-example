...
class ReactorAgentInput(BaseModel):
    ...
    ov_config: dict[str, Any] | None = Field(
        None, description='OpenViking config. None = local sqlite only.',
    )
...
class ReactorAgent(Routine):
    ...
    def _init(self, params: ReactorAgentInput) -> None:
        ...
        # 构造 ContextProvider (Local 或 Ov), 挂到 session.ctx.
        # replay items 暂存在 session._replay_items, response_id 状态在 tracker.
        self._ctx, self._ov_fs = build_context_provider(
            ov_config=params.ov_config,
            writer=self._writer,
    ...
    async def _on_input(self, source, data: dict) -> None:
        async with self._event_lock:
            from zero.routines._shared._agent_messaging import USER_SOURCE, wrap_from
