...
if TYPE_CHECKING:
    from .agent import ReactorAgent
    ...
    async def run(self, epoch: int, message_id: str) -> None:
        ...
        try:
            while max_turns is None or turn_count < max_turns:
                ...
                # OV 增量推送 (provider 内部按 N 轮计数 add_message, 不 commit;
                # 失败不阻断). 跟 react_agent 对齐: 每轮 tick 一次.
                if self._ctx is not None and self._ctx.enabled:
                    try:
                        self._ctx.tick()
                    except Exception as exc:
                        _log.warning('ov tick failed: %s', exc)
...
class ToolExecutor:
    """工具执行编排: 校验 → push → join → 写回 → feedback."""

    def __init__(self, agent: 'ReactorAgent') -> None:
        self._a = agent
        self._shell = agent._shell
        self._ctx = agent._ctx
    ...
    async def execute(
        ...
        for fc in known:
            ...
            h = await shell.push(fc.name, tool_args)
