...
万物皆 routine:UI 交互不散落在各 routine 自己发 ws 消息,而是 push 这个
``ask`` routine.它内部经 ``ctx.req(http_server_id, 'ui_request', ...)`` 走 wire
到 ``HttpServer`` 的 ``@request('ui_request')`` handler,HttpServer 广播
ui_request 给前端弹窗,等用户操作后返回 value.超时 / 出错不吞异常(向上传播,
让调用方决定语义--例如 ``dag_approval`` 必须 fail-closed,不能把超时误判成
有效答案).

入参结构对齐前端 wire 协议 ``{component, props}``:顶层 ``kind`` 选组件,
``props`` 放该组件的具体参数,``question`` / ``timeout`` 是公共字段.
...
用法(XML / 代码 / DAG 节点都行)::
    ...
    <ask question="选哪个?" options="a,b,c"/>
    <ask question="说点啥" options='["x"]' allow_other="true"/>
    <ask kind="date" question="你生日?" min="1900-01-01" max="2025-12-31"/>
...
不占模块:UI 弹窗不是硬件资源,前端 ``uiQueue`` 已串行展示(一次一个弹窗),
无需 kernel 模块互斥.
"""
...
# HttpServer 的 routine name(__init_subclass__ 蛇形转换 'HttpServer'
# -> 'web_server').ask 通过 ctx.get_running_routines 按名查 HttpServer id,
# 再经 ctx.req 走 wire 发起 UI 交互--不依赖模块级全局变量.
# (HttpServer 集成了原 AgentWSBridge 的 ui_request handler)
_BRIDGE_NAME = 'web_server'
# ctx.req 的 timeout 略大于 bridge 内部 ui_request timeout,确保 bridge 的
# timeout 回执能先到达 ask 端,ask 据此 raise TimeoutError(而非 ReqTimeout).
...
class Ask(Routine):
    ...
    async def _find_bridge_id(self) -> str | None:
        """查 HttpServer routine 当前 running 实例的 id(跨进程正确).

        HttpServer 是独立 passive routine(可能跑在另一个进程),ask 不持有它的
