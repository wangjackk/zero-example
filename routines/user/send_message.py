...
from zero.routines._shared._agent_messaging import PRIORITY_LOW
...
class SendMessageInput(BaseModel):
    ...
    to: str = Field(description='目标 agent 的 id (如 claude_1 / react_N)')
...
class SendMessage(Routine):
    ...
    用法 (XML / 代码)::
        ...
        <send_message to="claude_1" message="帮我看看这个文件"/>
        # 代码:
        result = await self.call('send_message', kwargs={
            'to': 'claude_1', 'message': '你好',
        })
        if result['ok']:
    ...
    meta: ClassVar[Dict[str, Any]] = {
        'description': (
            ...
            'to 指定目标 agent_id (如 claude_N / react_N).'
        ),
