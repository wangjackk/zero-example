...
由 ``server.HttpServer._start_shared`` 调用.设计成独立模块,让 server.py
只关心 routine 生命周期 + 框架事件订阅,@app 路由定义集中在此.

路由内通过 ``HttpServer._active`` 类级引用拿实例
...
def build_app(server_cls) -> FastAPI:
    """构造 FastAPI app.``server_cls`` 是 HttpServer 类(用其类级 _active/_handles)."""
    ...
    async def _run(name: str,
        ...
        inst = server_cls._active
        if inst is None:
            return _Json({'ok': False, 'error': 'no active HttpServer instance'})
        try:
    ...
    # ---- agents / dags: HTTP 等价物,复用 _on_* 内部方法 ----
    ...
    async def _run_ws_handler(handler, msg: dict) -> dict:
        inst = server_cls._active
        if inst is None:
            return {'ok': False, 'error': 'no active HttpServer instance'}
        box: dict = {}
    ...
    @app.get('/dags')
    async def _http_dags():
        """列出所有 DAG."""
        return _Json(await _run_ws_handler(_dag.on_dag_list, {}))

    @app.get('/dags/{name}')
    async def _http_dag_spec(name: str):
        """获取 DAG spec."""
        return _Json(await _run_ws_handler(_dag.on_dag_spec, {'dag': name}))
    ...
    async def _ws_endpoint(ws: WebSocket):
        ...
        inst = server_cls._active
        if inst is None:
            await ws.close(code=1011, reason='no active HttpServer instance')
            return
