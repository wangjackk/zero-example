"""HttpServer ---- HTTP + WS 前门 routine (FastAPI + uvicorn).

合并了原 HttpServer(HTTP 测试前门)和 AgentWSBridge(WS 桥接)的职责:
- HTTP 端点:``/health`` ``/routines`` ``/builtin_skills`` ``/run/{name}``
  ``/agents/{id}/run/{name}`` ``/agents*`` ``/dags*``
  ``/docs``(路由定义见 ``app.build_app``)
- WS 端点:``/ws``----前端主通道,处理 user_input/run/dag/agent/ui_request 等
  消息,广播 assistant_output/feedback/sys_prompt/session_changed 等框架事件

一个 uvicorn server 同时跑 HTTP + WS,端口默认 7780(``ZERO_HTTP_ADDR``).
``/routines`` 走 ``self.get_routines()``(kernel catalog 跨 hub 全量).

passive 自动起,类级单例复用 uvicorn server(kernel reconnect 不重绑端口).
拆分参考:_json / app / agents / dag 各管一摊,本模块只管 routine 生命周期 +
框架事件订阅 + WS 消息分发.
"""
...
class HttpServer(Routine):
    ...
    _active: Optional['HttpServer'] = None        # 最新 start 的 instance,路由用它投递
    ...
    async def on_started(self) -> None:
        # 旧版 bridge 在 on_started 挂 ws handler;现版 WS handler 是 FastAPI 路由,
        # 不需要挂.保留 hook 打日志.
        self._logger.info('[http/ws] bridge ready, ws clients: %d', len(self._ws_clients))

    async def run(self, kwargs: Dict[str, Any]):
        HttpServer._active = self            # 路由经类级 instance 投递
        if HttpServer._shared_server is None:
            self._start_shared()
        else:
    ...
    def _start_shared(self) -> None:
        addr = os.environ.get('ZERO_HTTP_ADDR', '127.0.0.1:7780')
        host, _, port = addr.rpartition(':')
        config = uvicorn.Config(
            build_app(HttpServer), host=host or '127.0.0.1', port=int(port),
            log_level='warning', access_log=False,
        )
        server = uvicorn.Server(config)
        server.install_signal_handlers = lambda: None  # type: ignore[method-assign]
        HttpServer._shared_server = server
        self._logger.info('🔌 http/ws server on http://%s  (ZERO_HTTP_ADDR=%s)',
                          addr, addr)
        async def _serve():
            try:
                await server.serve()
                ...
                self._logger.error('http/ws server startup failed (exit %s) -- '
                                   'port busy? ZERO_HTTP_ADDR=%s', exc.code, addr)
                HttpServer._shared_server = None
    ...
    @request('show_image')
    async def on_show_image(self, source, data: dict) -> dict:
        """routine -> bridge -> 前端展示图片 (fire-and-forget, 不等用户响应)."""
        agent_id = str(data.get('agent_id') or '')
        filename = str(data.get('filename') or '')
        img_len = len(str(data.get('image') or ''))
        self._logger.info('[bridge] show_image agent_id=%s filename=%s img_b64_len=%d',
                          agent_id, filename, img_len)
        await self._broadcast({
            'type': 'show_image',
            'agent_id': agent_id,
    ...
    @subscribe('dag_event')
    async def on_dag_event(self, source, data: dict) -> None:
    ...
    async def _on_client_message(self, msg: dict, reply) -> None:
        ...
        elif msg_type == 'dag_list':
            await _dag.on_dag_list(self, msg, reply)
        elif msg_type == 'dag_spec':
            await _dag.on_dag_spec(self, msg, reply)
        elif msg_type == 'dag_run':
            asyncio.create_task(_dag.on_dag_run(self, msg, reply))
    ...
    async def _on_run(self, msg: dict, reply) -> None:
        ...
        try:
            if fmt == 'form':
                ...
                xml = str(msg.get('xml') or '').strip()
                if not xml:
                    await reply({'type': 'run_result', 'id': req_id,
                                 'error': 'xml is empty'})
                    return
                result = await self.call('run_xml', {'xml': xml})
                await reply({'type': 'run_result', 'id': req_id, 'result': result})
            else:
                await reply({'type': 'run_result', 'id': req_id,
    ...
    async def _on_get_routines(self, reply) -> None:
        ...
        for r in routines:
            ...
            out.append({
                'name': r.get('name', ''),
                'is_passive': bool(r.get('is_passive', False)),
                'hub_id': r.get('hub_id', ''),
                'meta': meta,
            })
