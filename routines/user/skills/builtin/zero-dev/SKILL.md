---
name: zero-dev
description: zero 项目级开发指南(本仓特定)。在 zero 项目里写/改/注册 routine、查 routine 清单、理解目录结构或启动模式、用 Shell/DAG/claudecode/web_server 时使用。涵盖目录结构、routine 编写约定、业务 routine 清单、routine 管理(list/register/reload/deregister 经 run_routine 调)。
---
...
- **zero 是意识主体**:agent 自己不能重启 zero。zero 里只保留核心 agent(claudecode / react_agent / http_server 等意识主体基础设施),业务 routine 会逐步搬到 `one/`。
- **routine 聚合**:`zero/routines/__init__.py` 聚合 `test` / `user` 两组(后续只保留核心 agent,业务 routine 搬 one)。`one/routines/__init__.py` 是 routine 主战场。
...
- **HTTP+WS 前门**:`HttpServer` passive routine,一个 uvicorn server 同时跑 HTTP(curl 触发任意 routine:`/run/{name}` / `/start/{name}` / `/stop/{id}` / `/docs`)+ WS(`/ws` 端点,前端通信枢纽,框架事件 ↔ web 前端)。默认端口 7780。
- **编排器**:`zero/shell.py` 的 `Shell` 类,模块自动串并行(冲突→串行,否则→并行),`wait` 当 barrier。
- **agent 子系统**:`claudecode` 和 `react_agent` 走 manager+child 模式,passive manager spawn 非 passive child,通过 `agent_id` 隔离。
